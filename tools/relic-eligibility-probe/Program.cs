...
internal static class Program
{
    ...
    private static bool PolicyExcluded(string? triggerKind, EffectFragment effect) =>
        ...
        // Rule 9 (user order 2026-09-19): "prevent ineffective effects". A
        // combat-scoped effect on a post-combat trigger cannot do anything.
        // Reads the SAME sets the generator reads rather than re-listing opcodes,
        // so the oracle cannot drift from the implementation.
        || (GenerationSettings.Current.DisableIneffectiveEffects
            && triggerKind is not null
            && (triggerKind == "combat_end" || triggerKind == "combat_victory")
            && EffectFragment.CombatScopedOpcodes.Contains(effect.Opcode));
