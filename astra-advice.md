...
本节是方法提示, 不是新一轮玩法验证. 通用流程见 [总建议附录](../astra-advice.md).

1. **从交付物反推输入, 不从现有代码反推产品**. 先写一条能观察到的遗物路径: 获得 -> 持有 -> 事件/修改器执行 -> 保存 -> 恢复. 若当前设计仍必须传入正在打出的 CardModel 才能工作, 停下来审查抽象边界, 不用改名维持它.
2. **给每个原子做六项对账**: 何时触发, 谁触发, 作用给谁, 数值从哪里来, 满足哪些条件, 哪些状态跨事件保存. 任何一项丢失都不是无害压缩. 默认 self/0/null 只能在来源语义明确允许时使用.
3. **从命令实参反向追数据流**. 数值沿该次调用的实参追到变量/表达式, 不从整个函数找第一个数字. 目标沿实参追到最终对象, 不从函数里出现 HittableEnemies 就判全体攻击. 条件沿包围该调用的分支与跨回合状态追, 不只挑一个匹配词.
4. **先证明能重建来源, 再证明能打乱来源**. 先用独立来源期待值重建 BagOfMarbles/LetterOpener/ArtOfWar 等最小例, 然后检测生成的新配对不依赖原搭档. 两关缺一不可: 忠实复制不能证明重组, 重组成功也不能证明原子正确.
5. **区别参数角色, 不只比较类型**. 同为整数可能是阈值, 伤害, 回合数; 同为 modify_hand_draw 可能是 +2 或 -2; 同为 turn_start 可能在能量重置之前或之后. IR 若表达不出这种差别, 先修表达能力, 不让解释器猜来源类名.
6. **检查闭环而非只有前向依赖**. 失血触发失血, 获得触发再获得, 多件遗物互相触发, 都可能形成反馈. 要从已确认的原版限制与产品语义推导终止/限次规则, 不编造一个数字后称防崩完成.

最短复验顺序: 一条原生遗物忠实重建 -> 一个应拒绝的缺信息候选 -> 两个不同来源的新配对 -> 真实 RelicModel 持有执行 -> 重复查询不变 -> 新进程读档相同. 140 条/更多 opcode 只是覆盖盘点, 不是这六步的替代品.
